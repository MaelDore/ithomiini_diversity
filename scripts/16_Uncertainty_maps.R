...
##### Script 16: Map uncertainties in the modeling process #####
...
# 1/ Mean OMU uncertainty based on sd in ensemble models
...
# Inputs:
   # Layer of sd from each OMU ensemble model from script 10

# Ouputs:
   # Stack of sd of each OMU ensemble model
   # Map of mean OMU uncertainty
   # Stack of species richness from randomized selection of submodels
   # Map of species richness from randomized selection of submodels
...
library(tmaptools)
pal_grn_red <- rev(tmaptools::get_brewer_pal("RdYlGn", n = 200))
...
### 1.4/ Improved plot: Mollweide and graticule ####
...
{
  map_uncertainties_Mollweide <- function(x,                              # Raster to map
                                    color_palette = pal_grn_red,   # Color palette
...
Uncertainty_moll <- Mollweide_projection(mean_uncertainty_Jaccard.80)

pdf(file = paste0("./supplementaries/Ensemble_model_uncertainties.pdf"), height = 6, width = 6)

internal_margins <- par()$mar
# par(mar = c(3.1, 3.5, 3.5, 2.1))
par(mar = c(3.1, 3.1, 2.7, 1.6))

map_uncertainties_Mollweide(x = Uncertainty_moll,
                            main_title = "Ensemble model uncertainties",
                            legend_title = "Mean\nuncertainty",
                            legend_title_cex = 1.3,
                            legend_breaks = seq(0, 0.06, 0.02),
                            legend_title_x = -3050,
                            legend_title_y = 700,
                            arrow_scale = 0.6)
par(mar = internal_margins)
...
## 2.3.1 bis/ Improved plot ####

# Load map
mean_uncertainty_Jaccard.80 <- readRDS(file = paste0("./outputs/Uncertainties_maps/Random_richness_sd_Jaccard.80.rds"))
...
Species_richness_uncertainties_Moll <- Mollweide_projection(mean_uncertainty_Jaccard.80)

pdf(file = paste0("./supplementaries/Species_richness_uncertainties.pdf"), height = 6, width = 6)

internal_margins <- par()$mar
# par(mar = c(3.1, 3.5, 3.5, 2.1))
par(mar = c(3.1, 3.1, 2.7, 1.6))

map_uncertainties_Mollweide(x = Species_richness_uncertainties_Moll,
                            main_title = "Species richness uncertainties",
                            legend_title = "Uncertainty (sd)",
                            legend_title_cex = 1.3,
