...
sp.mean.rarity <- readRDS(file = "./outputs/Indices_maps/sp.mean.rarity_Jaccard.80.rds")
Faith.PD <- readRDS(file = "./outputs/Indices_Maps/PD.raster_Jaccard.80.rds")
FP <- readRDS(file = "./outputs/Indices_Maps/FP.raster_Jaccard.80.rds")
...
mimicry.mean.rarity <- readRDS(file = "./outputs/Indices_maps/mimicry.mean.rarity_Jaccard.80.rds")
load(file = "./outputs/Indices_maps/mean.ring.size.RData")
load(file = "./outputs/Indices_maps/weighted_mean_ring_size.RData")
load(file = "./outputs/Indices_maps/max_ring_size.RData")
load(file = "./outputs/Indices_maps/quantile_25_ring_size.RData")
load(file = "./outputs/Indices_maps/median_ring_size.RData")
load(file = "./outputs/Indices_maps/quantile_75_ring_size.RData")
load(file = "./outputs/Indices_maps/mean_ring_phenotypic_rarity.RData")
load(file = "./outputs/Indices_maps/mean_ring_prevalence.RData")
load(file = "./outputs/Indices_maps/mean_OMU_phenotypic_rarity.RData")
load(file = "./outputs/Indices_maps/vulnerability.RData")
load(file = "./outputs/Indices_maps/weighted_vulnerability.RData")
...
all_indices_stack <- stack(sp.richness, sp.diversity, sp.mean.rarity, Faith.PD, FP, MPD, ring.richness, ring.diversity, mimicry.mean.rarity, mean.ring.size, weighted_mean_ring_size, max_ring_size, quantile_25_ring_size, median_ring_size, quantile_75_ring_size, mean_ring_phenotypic_rarity, mean_ring_prevalence, mean_OMU_phenotypic_rarity, vulnerability, weighted_vulnerability)
...
names(all_indices_stack) <- index_list <- c("Sp. richness", "Sp. diversity", "Sp. geo. rarity", "Faith's PD", "ED", "MPD", "Mim. richness", "Mim. diversity", "Mim. geo. rarity", "Mean ring size", "Wg mean ring size", "Max ring size", "25% ring size", "Median ring size", "75% ring size", "Mean ring pheno. rarity", "Mean ring prev.", "Mean OMU pheno. rarity", "Vulnerability", "Wg vulnerability")
...
all_coords <- coordinates(select_indices_stack[[1]])

# Version with all communities
sample.all_index <- which(select_indices_stack[[1]][] > 0) 
...
# Sample indices
sample_indices_df <- data.frame(raster::extract(x = select_indices_stack, y = sample.all_coords))
...
# # Load stack
# select_indices_stack <- readRDS(file = "./outputs/Correlation_tests/select_indices_stack.rds")

### 1/ Approach with uncorrected Spearman's rank test ####

# With cor.test, easier, no need to synchronize the test
N.indices <- nlayers(select_indices_stack)
...
for (i in 1:N.indices)
{
  ...
  {
    test <- cor.test(x = select_indices_stack[[i]][], y = select_indices_stack[[j]][], method = "spearman", alternative = "two.sided", na.rm = T)
    rho_matrix[i,j] <- round(test$estimate, 3)
    rho_uncorrected_pvalues_matrix[i,j] <- round(test$p.value, 3)
  }
}

rownames(rho_matrix) <- colnames(rho_matrix) <- select_index_list 
rownames(rho_uncorrected_pvalues_matrix) <- colnames(rho_uncorrected_pvalues_matrix) <- select_index_list 
...
### 1.3/ Plot heatmap ####
pdf(file = paste0("./graphs/Correlation_tests/Heatmap_without_legend_", set, ".pdf"), height = 6, width = 6)
original_ext_margins <- par()$oma
par(oma = c(2,0,0,4))

heatmap(abs(rho_matrix), symm = T, Rowv = rev(dd_reorder), Colv = rev(dd_reorder), revC = T, col = heatmap_pal)
...
g1 <- ggplot(data = indices_df, aes(x = sp.richness, y = sp.mean.rarity)) +
  ...
  annotate("text", x = max(indices_df$sp.richness), y = max(indices_df$sp.mean.rarity), hjust = 0.9, vjust = 0.8, label = bquote(rho ~ "= 0.57"), size = 5, fontface = 2) +
  ...
  ylab(label = "Species mean rarity \n[Rarity index]") +
...
g4 <- ggplot(data = indices_df, aes(x = ring.richness, y = mimicry.mean.rarity)) +
  geom_point() +
  geom_smooth(color = "red", size = 1.5, se = F) +
  annotate("text", x = max(indices_df$ring.richness), y = min(indices_df$mimicry.mean.rarity), hjust = 1, vjust = 0.9, label = "d", size = 7, fontface = 2) +
  annotate("text", x = max(indices_df$ring.richness), y = max(indices_df$mimicry.mean.rarity), hjust = 0.9, vjust = -0.2, label = bquote(rho ~ "= 0.68"), size = 5, fontface = 2) +
...
Y1samp <- sample_indices_df$Sum.of.FP
...
rownames(rho_matrix) <- colnames(rho_matrix) <- index_list
rownames(rho_corrected_pvalues_matrix) <- colnames(rho_corrected_pvalues_matrix) <- index_list
rownames(rho_corrected_tstat_matrix) <- colnames(rho_corrected_tstat_matrix) <- index_list
rownames(rho_corrected_df_matrix) <- colnames(rho_corrected_df_matrix) <- index_list
...
rownames(rho_pvalue_matrix) <- colnames(rho_pvalue_matrix) <- select_index_list # Provide names for rows and columns
...
rownames(tstat_df_matrix) <- colnames(tstat_df_matrix) <- select_index_list # Provide names for rows and columns
...
rownames(Tjostheim_matrix) <- colnames(Tjostheim_matrix) <- index_list
...
Cor_indices_df <- as.data.frame(c("sp.richness", "sp.diversity.compatible_Jost", "mean.sp.rarity", "MPD.raster", "PD.raster", "FP.raster", "tot.ring.richness", "tot.ring.diversity.compatible_Jost", "mimicry.mean.rarity", "vulnerability", "HII.raster"))
