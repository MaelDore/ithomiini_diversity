...
###### Script 17: Threat curves ######

# Compute evolution of mean threat levels (Human Footprint) across proportions of most diverse/rare communities
...
# Save data for curve plot of mean HII
save(sp.richness_mean_values, sp.rarity_mean_values, MPD_mean_values, ring.richness_mean_values, ring.rarity_mean_values, file = "./outputs/Threat_curves/Most_diverse/most_diverse_rank_global_mean_HII_values.rds", version = "2")

# Save data for boxplots
save(sp.richness_boxplot_values, sp.rarity_boxplot_values, MPD_boxplot_values, ring.richness_boxplot_values, ring.rarity_boxplot_values, file = "./outputs/Threat_curves/Most_diverse/most_diverse_rank_global_boxplot_values.rds", version = "2")

# Save data for map masks
save(sp.richness_map_indices, sp.rarity_map_indices, MPD_map_indices, ring.richness_map_indices, ring.rarity_map_indices, file = "./outputs/Threat_curves/Most_diverse/most_diverse_rank_global_map_indices.rds", version = "2")
...
load(file = "./outputs/Threat_curves/Most_diverse/most_diverse_rank_global_mean_HII_values.rds")
...
### 5/ Plot to improve for SM (2.2.1) ####

### Curves plots for mean HII ~ ranks 
...
most_diverse_mean_df <- most_diverse_mean_df %>% 
  mutate(prop_com = (1:1000)/10) %>%    # Add a column for % of communities
  mutate(stat = "mean") %>%             # Add a column for stat computed
  filter(prop_com > 3) %>%              # Remove value for the top 3% because curves are idiosyncratic (nb of communities too low)
  pivot_longer(# cols = c("sp.richness", "sp.rarity", "MPD", "ring.richness"), # List of the name of the "columns" that are actually factor levels
    cols = c("sp.richness", "sp.rarity", "ring.richness", "ring.rarity"), # List of the name of the "columns" that are actually factor levels
    names_to = "index",   # name of the new column that is the real variable for these factor level
    values_to = "HII", # name of the new columns that will get the values in the cells of the previous dataset
...
limits_df <- tibble(x = c(0, 100), y = c(0, 20))
...
g <- ggplot(most_diverse_mean_df, aes(x = prop_com, y = HII)) +
  ...
  geom_hline(yintercept = mean(most_diverse_mean_df[most_diverse_mean_df$prop_com == 100, "HII", drop = T]), # Get the final mean HII for 100% of communities
             lwd = 1.2) +
  geom_vline(xintercept = c(5, 25), # Draw the threshold for the boxplots
             col = "grey50", lty = 2, lwd = 1) +
  
  labs(y = "Mean Human Footprint  [%]", x = "Proportion of communities  [%]") +
  
  # expand_limits(y = 0, x = 0) + # To tell to always keep those values inside the plot. # Set the origin for the graph
  
  ggthemes::geom_rangeframe(data = limits_df, aes(x = x, y = y), size = 1.4) +
  
  scale_x_continuous(expand = expansion(mult = c(0, 0)), breaks = c(5, 25, 50, 75, 100)) + 
  scale_y_continuous(expand = expansion(mult = c(0, .05))) + # To tell not to add padding around the limits of the axis
  
  scale_color_manual(values = c("sp.richness" = "#FF4040", "sp.rarity" = "#80C2FF", "ring.richness" = "goldenrod1", "ring.rarity" = "#7ACC29"),
                     name = "Indices", labels = c("Species richness", "Species rarity", "Mimicry richness", "Mimicry rarity")) +
  ...
  theme(panel.background = element_rect(fill = "white", color = "NA"),
        ...
        legend.text = element_text(size = 12, face = "plain", 
                                   margin = margin(t = -5, unit = "pt")), # To reduce space between legend items
        # margin = margin(t = 0, unit = "pt")),
        legend.key.size = unit(c(2.5, 0), 'lines'),
        legend.spacing.y = unit(-0.3,"cm"),  # Should reduce space between legend items but in the end, reduce space with legend title
        axis.ticks = element_line(size = 1, color = "black"),
        axis.ticks.length = unit(8, "pt"),
        axis.text = element_text(size = 14, face = "bold", color = "black"),
        axis.text.x.bottom = element_text(vjust = -0.5),
        # axis.text.y.left = element_text(hjust = -0.2),
...
dev.off()
file.copy(from = paste0("./graphs/Threat_curves/Most_diverse/Most_diverse_rank_global_mean.pdf"), to = paste0("./supplementaries/Threat_curves.pdf"), overwrite = T)
