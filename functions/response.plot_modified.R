...
# =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= #
`response.plot3` <- function(models, 
                             Data, 
...
#   Data = envData[[cur.vars]]
#   show.variables = cur.vars[c(7, 8, 9)]
#   do.bivariate = T
#   fixed.var.metric = 'mean'
#   save.file="no"
#   name="response_curve"
  ...
  for(i in 1:ncol(Data)){
    if(is.numeric(Data[,i])){
      Data.r[,i] <- switch(fixed.var.metric,
                           ...
                           sp.mean = rep(mean(run.data@data.env.var[which(run.data@data.species == 1), i]),  nb.pts))
    } else{
      Data.r[,i] <- switch(fixed.var.metric,
