# Only change in the code at the url level allowing the control of the API key

customed_gmap <- function (x, exp = 1, type = "terrain", filename = "", style = NULL, 
          ...
          lonlat = FALSE, key, ...) 
{
  ...
  gurl <- paste(gurl, "center=", ctr, "&zoom=", zoom, "&size=", s, 
                "&maptype=", type,"&key=", key, # Only change in the code allowing the control of the API key
                "&format=gif&sensor=false&scale=", 
                scale, sep = "")
  if (!is.null(style)) {
    style <- gsub("\\|", "%7C", style)
    style <- gsub(" ", "", style)
