...
# list_sp_regions <- list(Caatinga_shp, Guyana_Shield_shp)
# region_names <- c("Caatinga", "Guyana_Shield")

# list_sp_regions <- list(Central_Andes_shp, Western_Amazon_shp)
...
### 20.4.1/ Function to compute SES-MR according to OMU-richness

### Change for SR but specify can be used at OMU level if polymorphism ####

compute_SES_MR_vs_OMU_richness <- function (proba_stack, # Continuous (or binary) SDM outputs of species/OMU
                                            mimicry_ring_membership, # Dataframe providing the ring membership of species/OMU
                                            seed = 1, # Set seed to ensure reproducibility
                                            randomizations = 999, # To select the number of randomization
                                            alpha = 0.05,  # Threshold for significance for a two-sided test
                                            ...
                                            example_coords = NULL) # To provide the coordinates of the community to use as example, otherwise a community with median species richness is used.
{
  ...
  ### Need to include/create a function to generate mimicry ring probability and richness stacks from a species/OMU stack and ring membership table
  
  
  ### Then use the mimicry richness function to compute MR from ring_proba_stack
...
SES_MR_vs_OMU_richness <- compute_SES_MR_vs_OMU_richness(sp_proba_stack, phylo, seed = 1,
                                                         randomizations = 99, # To select the number of randomization
                                                         alpha = 0.05,  # Thresold for significance for a two-sided test
                                                         export_null_rasters = F, # Include the raster Stack of PD obtained from randomization in the output
                                                         plot_distri = T) # To plot the null distribution for an "average" community as an example
...
# Save
save(SES_MR_vs_OMU_richness, file = paste0("./outputs/Indices_maps/SES_MR_vs_OMU_richness.RData"))
